...
function Util.pickRandom<T>(tbl: { T }, except: T | nil): T | nil
...
function Util.weightedRandom(values, weights)
	...
	for i = 1, #weights do
		cursor += weights[i]
		if cursor >= random then return values[i] end
	end
...
function Util.filter(
	...
	for key, value in t do
		if predicate(key, value) then table.insert(newTable, value) end
	end
...
function Util.filter_map_dict<T, U>(
	t: { [T]: U },
...
function Util.filter_map<T, U>(t: { [T]: U }, filter_mapper: (T, U) -> any)
	...
	for key, value in t do
		local mapped = filter_mapper(key, value)
		if mapped then table.insert(newTable, mapped) end
	end
...
function Util.scaleNumberSequence(
	...
	for _, keyPoint in numberSequence.Keypoints do
		table.insert(
			...
			NumberSequenceKeypoint.new(
				keyPoint.Time,
				keyPoint.Value * scaleFactor,
				keyPoint.Envelope * scaleFactor
			)
		)
	end
...
function Util.TweenModelCF(model: Model, tweenInfo: TweenInfo, goal): Tween
...
function Util.TweenModelSize(model: Model, tweenInfo: TweenInfo, goal): Tween
	local size = Instance.new("NumberValue")

	size.Value = model:GetScale()
	size.Changed:Connect(function()
		model:ScaleTo(size.Value)
	end)

	goal.Value = goal.Scale
	goal.Scale = nil

	return TweenService:Create(size, tweenInfo, goal)
end
...
function Util.makePartBetween(a: Vector3, b: Vector3): Part
	...
	part.CFrame = CFrame.new(b + vector / 2, a)
		* CFrame.new(-part.Size.X / 2, 0, 0)
	return part
end
...
function Util.formatNumber(x: number): string
